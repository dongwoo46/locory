...
export default function MapClient({ userId }: Props) {
  ...
  const [mapMode, setMapMode] = useState<
    'normal' | 'course-build' | 'course-view'
  >('normal');
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-10 pointer-events-none">
          <div className="max-w-lg mx-auto px-3 pt-3 flex flex-col gap-2">
            ...
            {mapMode === 'normal' && showFilters && (
              <div
                className="bg-white rounded-2xl shadow-lg p-3 pointer-events-auto flex flex-col gap-3 overflow-y-auto"
                  ...
                  <div className="flex gap-1.5">
                    {(
                      [
                        ...
                        { key: 'feed', label: t('viewFeed') },
                        { key: 'places', label: t('viewPlaces') },
                      ] as const
                    ).map((v) => (
                      <button
                        key={v.key}
                ...
                {hasActiveFilters && (
                  <div className="flex flex-wrap gap-1.5">
                    {viewMode !== 'all' && (
                      ...
                      >
                        {viewMode === 'feed' ? t('viewFeed') : t('viewPlaces')}
                        <svg
                          width="10"
                          height="10"
                          fill="none"
                    ...
                    {minRating != null && (
                      ...
                      >
                        {tPost(
                          `rating.${minRating === 4 ? 'must_go' : minRating === 3 ? 'worth_it' : 'neutral'}`,
                        )}{' '}
                        {t('above')}
                        <svg
                          width="10"
                ...
                <div>
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    ...
                    {[
                      {
                        score: 4,
                        label: tPost('rating.must_go'),
                        color: RATING_COLORS.must_go,
                      },
                      {
                        score: 3,
                        label: tPost('rating.worth_it'),
                        color: RATING_COLORS.worth_it,
                      },
                      {
                        score: 2,
                        label: tPost('rating.neutral'),
                        color: RATING_COLORS.neutral,
                      },
                    ].map((r) => (
                      <button
                        key={r.score}
                        onClick={() =>
                          setMinRating(minRating === r.score ? null : r.score)
                        }
                    ...
                    {allCategories.map((cat) => (
                      ...
                      >
                        <span
                          ...
                          style={{
                            backgroundColor: categories.has(cat)
                              ? 'white'
                              : CATEGORY_COLOR[cat],
                          }}
                        />
          ...
          <div className="max-w-lg mx-auto px-3 pt-3">
            <div className="bg-white rounded-2xl shadow-lg px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  {t('course.dayPlaces', { days: courseSettings.days, count: courseSelection.length })}
                </p>
              </div>
              <button
                onClick={() => {
      ...
      {mapMode === 'course-build' && buildStep === 'settings' && (
        ...
        >
          <div
            className="bg-white w-full max-w-lg rounded-t-2xl max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-center pt-3 pb-1">
              <div className="w-8 h-1 bg-gray-200 rounded-full" />
            ...
            <div className="px-4 pb-24 pt-2 flex flex-col gap-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(
                    [
                      { value: 'relaxed', label: t('course.styleRelaxed') },
                      { value: 'packed', label: t('course.stylePacked') },
                      { value: 'food', label: t('course.styleFood') },
                      { value: 'photo', label: t('course.stylePhoto') },
                      { value: 'healing', label: t('course.styleHealing') },
                    ] as const
                  ).map((opt) => (
                    <button
                      key={opt.value}
                      onClick={() =>
                        setCourseSettings((s) => ({ ...s, style: opt.value }))
                      }
                ...
                <div className="flex gap-2">
                  {[9, 10, 11, 12].map((h) => (
                    <button
                      key={h}
                      onClick={() =>
                        setCourseSettings((s) => ({ ...s, startHour: h }))
                      }
                  ...
                  {savedAccommodation && (
                    <button
                      onClick={() =>
                        setCourseSettings((s) => ({
                          ...s,
                          startLocation: savedAccommodation.name,
                        }))
                      }
                  ...
                  {savedAccommodation && (
                    <button
                      onClick={() =>
                        setCourseSettings((s) => ({
                          ...s,
                          endLocation: savedAccommodation.name,
                        }))
                      }
                      className="shrink-0 px-3 py-2 bg-gray-100 text-gray-600 rounded-xl text-xs font-medium whitespace-nowrap"
                    >
