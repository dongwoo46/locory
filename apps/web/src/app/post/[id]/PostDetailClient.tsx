    ...
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-40 border-b border-gray-100 bg-white">
        <div className="mx-auto flex h-12 w-full max-w-lg items-center px-4">
          ...
          <h1 className="flex-1 text-center text-sm font-semibold text-gray-900">{post.places?.name ?? ''}</h1>
          <div className="w-6" />
        </div>
      </header>
        ...
        <div className="flex items-center gap-2.5 px-4 py-3">
          ...
          >
            {post.profiles?.avatar_url ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-xs text-gray-400">{post.profiles?.nickname?.[0]}</div>
            )}
          </button>
          <div className="min-w-0 flex-1">
            ...
            <button
              onClick={() => router.push(`/place/${post.places?.id}`)}
              ...
              {comments.map((c) => {
                ...
                return (
                  <div key={c.id} className="flex items-start gap-2">
                    ...
                    {c.user_id === userId && (
                      <button onClick={() => deleteComment(c.id)} className="mt-0.5 shrink-0 text-[10px] leading-none text-gray-300">
                        {tPost('delete')}
                      </button>
                    )}
                  </div>
